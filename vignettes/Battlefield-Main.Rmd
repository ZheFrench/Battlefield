...
`Battlefield` provides three core functionalities to define and extract 
...
1. Detect interfaces between two specific clusters (inside/outside region)
...
We first load a simulated Visium dataset (square binned) on and then we process 
another Visium Standard dataset (hexagonal grid) at
lower resolution to illustrate the method.
...
set.seed(42)
inner_A <- select_inner_spots(df_visium, all_borders_visium, 
                               cluster = cluster_A, interface = cluster_B, 
                               region = "both")
