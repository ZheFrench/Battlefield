...
#'
#' @param cluster_labels A vector of cluster labels (character, factor, numeric,
#'   etc.).
...
pairs$directed_pair <- paste0(pairs$cluster, interface_separator,
pairs$interface)
rownames(pairs) <- NULL
...
#' a k-nearest-neighbors neighborhood (optionally constrained by a distance
#' cutoff).
#'
#' In addition, it flags spots that also touch **other clusters** (i.e.,
#' junction
...
#' @param cluster Cluster label for the source cluster (A). Spots in this
#' cluster
...
#' - "outer": returns border spots from interface → cluster (swaps
#' cluster/interface).
...
#' @param max_dist Optional numeric. Maximum Euclidean distance for a neighbor
#'   to be considered (distance cutoff). If `NULL`, no distance filtering is
#'   applied.
...
#' \item Marks a spot as a border spot if it has at least one neighbor in
#' `interface`.
#' \item Flags a spot as multi-interface if it also has a neighbor belonging to
#' a
#'   cluster different from `cluster` and `interface`.
#' }
#'
#' **Note on mode column**: The `mode` column in the returned data.frame will
#' always contain
#' either "inner" or "outer", even if the `mode` parameter is set to "both".
#' When `mode="both"`,
#' the function internally calls the selection algorithm twice (once for
#' cluster→interface and
#' once for interface→cluster), then row-binds the results, so each row is
#' labeled with its
#' actual direction.
#'
...
#' \item{directed_pair}{Interface label combining cluster and `interface`, e.g.
#' `"A-B"`.}
#' \item{undirected_pair}{Undirected pair label (both directions), e.g. `"A-B /
#' B-A"`.}
...
#' \item{other_adjacent_borders}{Comma-separated list of other clusters touched,
#' or `NA`.}
...
#'
#' This function iterates over all **ordered** cluster pairs (A -> B) and
#' returns
#' a single data.frame containing the border spots for each interface, as
#' computed
...
#'
#' @param df A data.frame containing at least coordinate columns and a cluster
#' label column.
#' @param k Integer. Number of nearest neighbors to consider (excluding self)
#' when
#'   computing borders. Default is 6.
#' @param max_dist Optional numeric. Maximum Euclidean distance for neighbors to
#' be
#'   considered. If `NULL`, no distance filtering is applied.
#' @param mode Character. One of "inner", "outer", or "both". Controls which
#' border
#'   direction(s) to select for each pair. Default is "both".
#' @param pairs Optional data.frame of oriented cluster pairs, typically
#' produced by
...
#'   Default is `"cluster"`.
#'
#' @return A data.frame produced by row-binding the result of border selection
#' for
#' each oriented pair. Typically contains the original columns of `df` plus
#' interface
#' annotation columns from the border selector.
#'
#' **Note on mode column**: The `mode` column in the returned data.frame will
#' always contain
#' either "inner" or "outer", never "both". Even when the `mode` parameter is
#' set to "both",
#' each border spot row is labeled with its actual direction (cluster→interface
#' is "inner",
...
#' This function iterates over all **ordered** cluster pairs (A -> B) and
#' returns
#' a single data.frame containing the core (control) spots for each pair, as
#' computed
...
#'   Default is `"cluster"`.
#'
#' @return A data.frame produced by row-binding the result of core spot
#' selection for
#' each oriented pair. Typically contains the original columns of `df` plus
#' annotation
#' columns (is_core, interface, mode) from the core spot selector.
#'
#' **Note on mode column**: The `mode` column in the returned data.frame
#' reflects which
#' border modes were used for each core spot selection: "inner", "outer", or
#' "both".
#' This differs from `build_all_borders()` which only returns "inner" or
#' "outer".
#'
#' @examples
#' # Example with synthetic data
...
if (is.null(pairs)) {
    ...
    warn <- paste0("Warning: ", invalid_count,
    " provided pair(s) not found in border_df.    Filtering them out.")
    warning(warn)
...
#' read from the `border_df` dataframe for this cluster/interface pair. If
#' multiple
#' mode values exist in border_df for the pair, a warning is printed and the
#' first value is used.
#'
...
#'
#' This returns N random inner spots from the cluster, where N equals the border
#' count.
#'
#' @param df A data.frame containing at least the coordinate columns and a
#' cluster label column.
...
#' @param cluster Character. Label of the cluster from which to select inner
#' spots.
#' @param interface Character. Label of the target cluster for the directed
#' pair.
#' @param mode Character. One of "inner", "outer", or "both". Controls which
#' mode values
#' from border_df to use when counting border spots. When mode="both", both
#' "inner" and
...
#'
#' @return A data.frame of sampled core spots from `cluster`, with all columns
...
if (!pair_exists) {
    warn <- paste(
    "No border spots found for pair",
    "(", cluster, ", ", interface, ")",
    ". Returning NULL.",
    sep = "") 
    warning(warn)
...
pair_border_df <- border_df_filtered[(border_df_filtered$cluster == cluster & 
border_df_filtered$interface == interface) |
...
if (n_core < border_count) {
    msg <- paste(
        "Not enough core spots for pair",
        "(cluster=", cluster,
        ", interface=", interface,
        ", mode=", mode, ")",
        ". Requested:", border_count,
        ", Available:", n_core,
