...
#' - `border_mode`: character, "inner", "outer", or NA (from the `mode` column in input data)
#'
...
#' spe <- add_frontlines_to_spe(spe, border = all_borders, core = all_cores)
#'
#' # Or using individual select functions
#' border_3_to_4 <- select_border_spots(df, cluster = 3, interface = 4, k = 6)
#' core_3_to_4 <- select_core_spots(df, all_borders, cluster = 3, interface = 4, region = "inner")
#' spe <- add_frontlines_to_spe(spe, border = border_3_to_4, core = core_3_to_4)
#'
#' @importFrom SummarizedExperiment colData "colData<-"
#' @importFrom methods is
#' @export
add_frontlines_to_spe <- function(spe,
                                   border = NULL,
                                   core = NULL,
                                   erase = FALSE) {
  ...
  if (length(existing_battlefield_cols) > 0) {
    if (!erase) {
      message("Battlefield columns already exist: ", paste(existing_battlefield_cols, collapse = ", "),
...
#' and adds them as annotations in the colData of a SpatialExperiment object.
#'
#' @param spe A SpatialExperiment object from which the layer dataframes were derived.
...
#'
#' This function takes trajectory results (e.g., from [build_similar_trajectories()])
#' and adds trajectory metadata to the colData of a SpatialExperiment object.
#'
#' @param spe A SpatialExperiment object from which the trajectory dataframes were derived.
#' @param trajectory A data.frame of trajectory spots. Expected output from
#'   [build_similar_trajectories()]. Expected columns: spot_id, trajectory_id, offset, pos_on_seg, dist_to_seg.
...
#' - `trajectory_id`: character, identifier for each trajectory (e.g., "main", "left_1", "right_2")
...
#' trajs <- build_similar_trajectories(df, A, B, top_n = 10, n_extra = 1, side = "both")
...
add_trajectories_to_spe <- function(spe,
  ...
  if (length(existing_trajectory_cols) > 0) {
    if (!erase) {
      message("Trajectory columns already exist: ", paste(existing_trajectory_cols, collapse = ", "),
              ". Set erase=TRUE to overwrite them.")
      return(spe)
    } else {
