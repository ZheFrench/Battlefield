...
#' @param intermediate_quantile Numeric between 0 and 1. The quantile of distances to border
#'   used to define the intermediate layer threshold. Default is 0.5 (median). Spots with
#'   distance to border <= this quantile threshold are classified as intermediate.
#'   Use lower values (e.g., 0.33) for narrower intermediate layers, higher values (e.g., 0.75)
...
#' @details
#' Layer definitions:
#' - **Border**: Spots in the target cluster that touch at least one spot from a different cluster (kNN-based).
#' - **Intermediate**: Non-border spots whose distance to the nearest border spot is within
...
#'
#' The `intermediate_quantile` parameter controls the depth of the intermediate layer:
...
#' layers_narrow <- create_cluster_layers(df, target_cluster = 1, k = 6, intermediate_quantile = 0.33)
...
create_cluster_layers <- function(df,
  ...
  # A spot is a border spot if it touches at least one spot from a different cluster
  touches_other <- apply(!(neigh_cl == target_cluster) & !is.na(neigh_cl),
                         1, any)

  border_spot_ids <- target_df$spot_id[touches_other]

  # Identify intermediate spots: spots neighboring border spots but not themselves borders
  ...
  if (length(non_border_idx) == 0) {
    ...
    message("Cluster ", target_cluster, ": Only 'border' layer detected. All spots are at cluster interfaces.")
  } else {
    # Query non-border spots for neighbors in border region
    nn_check <- RANN::nn2(data = coords[border_spot_indices_global, , drop = FALSE],
                          query = coords[non_border_idx, , drop = FALSE],
                          k = 1)
    ...
    depth_threshold <- stats::quantile(distances_to_border, intermediate_quantile)
    ...
    if (sum(target_df$layer == "core") == 0) {
      ...
      if (sum(target_df$layer == "border") > 0 && sum(target_df$layer == "intermediate") > 0) {
        # Transform intermediate spots to core
        target_df$layer[target_df$layer == "intermediate"] <- "core"
        message("Cluster ", target_cluster, ": Only 'border' and 'intermediate' layers detected. ",
                "Setting 'intermediate' spots to 'core'.")
      }
    }
  }
...
#'   Default is `"cluster"`.
#'
#' @return A data.frame combining all processed clusters with the `layer` column added.
#'
#' @examples
#' data("visiumHD_16um_simulated_spe", package = "Battlefield")
#' spe <- visiumHD_16um_simulated_spe
