...
#' @param coords Character vector of length 2 giving the coordinate column names.
#'   Default is `c("x","y")`.
...
#' @param verbose Logical. If `TRUE`, prints diagnostic messages. Default is `TRUE`.
#'
#' @details
#' The grid step size is estimated as the median of the per-point nearest-neighbor
...
#'   \item{median_ratio}{Median normalized distance ratio in the (1.2, 2.0) range.}
#' }
#'
#' @examples
...
#' hex <- hex[, c("x","y")]
#' res_hex <- detect_grid_type(hex, coords = c("x","y"), k = 12, tolerance = 0.1)
#' res_hex$grid_type
#'
#' @importFrom RANN nn2
#' @export
...
#' @param coords Character vector of length 2 giving the coordinate column names.
#'   Default is `c("x","y")`.
#' @param square_connectivity Integer-like choice for square grids: `4` (Von Neumann)
#'   or `8` (Moore). Default is `c(4, 8)` which selects the first value (`4`).
#' @param tolerance Numeric. Passed to [detect_grid_type()] to match characteristic
...
#' Returned values:
#' \describe{
#'   \item{grid_type}{Detected grid type: `"hexagonal"`, `"square"`, or `"unknown"`.}
#'   \item{connectivity}{Nominal grid connectivity (6 for hex, 4 or 8 for square).}
...
#' Notes:
#' \itemize{
#'   \item `radius` is set to `1.01 * step` (or `1.01 * sqrt(2) * step` for 8-connectivity)
...
#' }
#'
#' @return A named list with neighborhood parameters: `grid_type`, `connectivity`,
